...
-- Event handler
function CT_RATarget_OnEvent(self, event, ...)
...
-- Update handler
function CT_RATarget_OnUpdate(self, elapsed)
...
-- Loading procedure
function CT_RATarget_OnLoad(self)
	self.elapsed = 0.5;
	self:SetBackdropColor(0, 0, 0, 0.8);
	self:RegisterEvent("GROUP_ROSTER_UPDATE");
	self:RegisterEvent("PLAYER_TARGET_CHANGED");
	for i = 1, 10, 1 do
		_G[self:GetName() .. "MT" .. i .. "Text"]:SetText("MT #" .. i .. ":");
		_G[self:GetName() .. "Target" .. i .. "Text"]:SetText("Target #" .. i .. ":");
	end
	CT_RATarget_UpdateInfoBox();
end
	...
	elseif ( UnitExists("target") and UnitInRaid("player") ) then
		if ( UnitCanAttack("player", "target") and UnitInRaid("player") ) then
			...
			if ( var.MainTanks ) then
				...
				for k, v in pairs(var.MainTanks) do
					if ( v[1] ) then
			...
			if ( hasTargetAsTarget and UnitName("player") ~= hasTargetAsTarget[2] ) then
				...
				if ( hasSeveralAsTarget ) then
					local tempText = "";
					for k, v in pairs(hasSeveralAsTarget) do
...
function CT_RATarget_UpdateStats()
	...
	local errors = { };
	for i = 1, 10, 1 do
		_G["CT_RATargetFrameTarget" .. i .. "Error"]:Hide();
		local mtText = _G["CT_RATargetFrameMT" .. i .. "Text"];
		local targetText = _G["CT_RATargetFrameTarget" .. i .. "Text"];
		if ( CT_RATarget.MainTanks[i] ) then
			...
			_G["CT_RATargetFrameTarget" .. i].lock = nil;
			if ( _G["CT_RATargetFrameTarget" .. i].isOver ) then
				_G["CT_RATargetFrameTarget" .. i .. "MouseOver"]:Show();
			end
			...
			_G["CT_RATargetFrameTarget" .. i .. "MouseOver"]:Hide();
			_G["CT_RATargetFrameTarget" .. i].lock = 1;
	...
	for k, v in pairs(errors) do
		if ( id < 5 ) then
			...
			for key, val in pairs(v) do
				_G["CT_RATargetFrameTarget" .. val .. "Error"]:Show();
				_G["CT_RATargetFrameTarget" .. val .. "Error"]:SetVertexColor(errorColors[id][1], errorColors[id][2], errorColors[id][3]);
			end
			_G["CT_RATargetFrameTarget" .. k .. "Error"]:Show();
			_G["CT_RATargetFrameTarget" .. k .. "Error"]:SetVertexColor(errorColors[id][1], errorColors[id][2], errorColors[id][3]);
		end
	end
end

function CT_RATarget_SetMT(id, frame)
	if ( UnitExists("target") and UnitInRaid("target") ) then
		for k, v in pairs(CT_RATarget.MainTanks) do
			if ( v[2] == UnitName("target") ) then
				CT_RATarget_TargetMT(id, frame);
				return;
			end
		end
...
function CT_RATarget_TargetMT(id, frame)
	if ( CT_RATarget.MainTanks[id] ) then
		frame:SetAttribute("unit", "raid" .. CT_RATarget.MainTanks[id][1]);
		CT_RATarget_UpdateInfoBox();
	end
end

function CT_RATarget_AssistMT(id, frame)
	if ( CT_RATarget.MainTanks[id] ) then
		frame:SetAttribute("unit", "raid" .. CT_RATarget.MainTanks[id][1] .. "target");
		CT_RATarget_UpdateInfoBox();
	end
end
...
function CT_RATarget_AssistButton_OnClick(self)
	for i = 1, GetNumRaidMembers(), 1 do
